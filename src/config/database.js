...
async function ensureCollections(database) {
  ...
  await ensureCollection(database, 'settings', settingValidator);
  ...
  await database.collection('settings').updateOne(
    { _id: 'rental-policy' },
    {
      $setOnInsert: {
        ...
        discounts: [{ minimumItems: 3, percentage: 5 }, { minimumItems: 6, percentage: 10 }]
      }
    },
    { upsert: true }
  );
...
const customerValidator = { bsonType: 'object', required: ['fullName', 'phone', 'email', 'birthDate', 'registeredAt', 'blocked'], properties: { fullName: { bsonType: 'string', minLength: 1 }, phone: { bsonType: 'string' }, email: { bsonType: 'string' }, birthDate: date, registeredAt: date, blocked: { bsonType: 'bool' }, address: { bsonType: 'object' }, location: { bsonType: 'object', properties: { type: { enum: ['Point'] }, coordinates: { bsonType: 'array', minItems: 2, maxItems: 2 } } } } };
...
const settingValidator = { bsonType: 'object', required: ['maxDays', 'ratesByDays', 'discounts'], properties: { maxDays: { bsonType: 'int', minimum: 1 }, ratesByDays: { bsonType: 'array', minItems: 1 }, discounts: { bsonType: 'array' } } };